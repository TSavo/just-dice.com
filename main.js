...
var timer_num = function(){return 3000 + (Math.random() * 3000)}; // Timer delay between bets.
...
function martingale() {
	...
	if (bal.data('oldVal') != bal.val() && running) {
		...
		// Add a step into the martingale to see if we reach our desired loss
		// length, If so reset
		if (current_bet_num == $delay.val()) {
			...
			current_steps = 1;
			current_bet_num = 0;
			$("#pct_bet").val(start_bet + (Math.random() * 0.000001));

			// Increase our bet by the multiplier
			var new_val = $("#pct_bet").val(); // Why I had left a multiplyer
												// here.. Madness Fixed now.
		...
		else if ($.isNumeric($multiplier.val()) && $.isNumeric($steps.val())
				&& (current_steps < $steps.val())) {

			// Increase our bet by the multiplier
			var new_val = $("#pct_bet").val() * $multiplier.val();
			$multiplier.val(2 + (Math.random() * 0.5));
			// get rid of scientific notation
			if (String(new_val).indexOf('e') !== -1) {
				var arr = new Array();
				arr = String(new_val).split('e');
...
function tabber() {
	...
	$('<li>').append($('<a>').text('Nix-Tab').attr('href', '#Nixsy9'))
			.appendTo('.tabs');
};
